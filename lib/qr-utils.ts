...
export function generateClaimCode(eventId: string, index: number): string {
  ...
  return `FLOW-${eventId.slice(0, 4).toUpperCase()}-${timestamp}-${random}`
}
...
export function isClaimCodeValid(claimCode: string): boolean {
  // Validate claim code format: FLOW-XXXX-XXXXXXXXX-XXXXXX
  const pattern = /^FLOW-[A-Z0-9]+-[A-Z0-9]+-[A-Z0-9]+$/
